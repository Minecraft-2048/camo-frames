 ...
 */
public class FramedBitsItem extends BlockItem {
	public FramedBitsItem(net.minecraft.world.level.block.Block block, Properties props) {
		super(block, props);
	}
	...
	public InteractionResult useOn(UseOnContext ctx) {
		...
		if (level.getBlockState(clicked).is(ModBlocks.FRAMED_BITS)) {
			int oct = FramedBitsBlock.addOctant(clicked, hit);
	...
	private InteractionResult placeAdjacent(UseOnContext ctx, BlockPos clicked, Direction side, BlockHitResult hit) {
		...
		if (cur.is(ModBlocks.FRAMED_BITS)) {
			return fill(ctx, target, FramedBitsBlock.clickedOctant(target, hit));
		}
		if (!cur.canBeReplaced()) return InteractionResult.PASS;
		int oct = FramedBitsBlock.firstOctant(side, ctx.getClickLocation(), clicked);
		if (!level.isClientSide()) {
			level.setBlockAndUpdate(target, ModBlocks.FRAMED_BITS.defaultBlockState());
		}
