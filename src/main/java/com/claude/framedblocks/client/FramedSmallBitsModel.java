...
/**
 * Renders a framed-bits block: each present cell (4x4x4 px sub-cube) emits its
 * EXPOSED faces (a face is skipped when the neighbouring octant is also present)
 * textured with that octant's camo via geometry projection -- so 64 identical
 * cells tile into a seamless full block. Bare (AIR) octants use the frame sprite.
 */
public class FramedSmallBitsModel extends WrapperBlockStateModel {
	private static final Direction[] DIRS = Direction.values();

	private record Layer(float u0, float u1, float v0, float v1, int color, ChunkSectionLayer chunkLayer) {}

	public FramedSmallBitsModel(BlockStateModel wrapped) {
		super(wrapped);
	}
	...
	public void emitQuads(QuadEmitter emitter, BlockAndTintGetter blockView, BlockPos pos,
		...
		if (!(data instanceof BlockState[] octants) || octants.length != 64) return;
	...
	private void emitFace(QuadEmitter emitter, BlockAndTintGetter blockView, BlockPos pos,
		...
		float x0 = ox * 0.25f, x1 = x0 + 0.25f;
		float y0 = oy * 0.25f, y1 = y0 + 0.25f;
		float z0 = oz * 0.25f, z1 = z0 + 0.25f;
	...
	private static int neighbor(int i, Direction d) {
		int x = (i & 3) + d.getStepX();
		int y = ((i >> 2) & 3) + d.getStepY();
		int z = ((i >> 4) & 3) + d.getStepZ();
		if (x < 0 || x > 3 || y < 0 || y > 3 || z < 0 || z > 3) return -1;
		return x | (y << 2) | (z << 4);
	}
